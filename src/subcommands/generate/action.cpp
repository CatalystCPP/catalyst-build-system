...
GeneratedVariables writeVariables(const catalyst::utils::yaml::Configuration &config,
                                  catalyst::generate::buildwriters::BaseWriter &writer,
                                  const std::vector<FeatureFlag> &features, /// < consume new FeatureFlag struct instead
                                                                            /// of just the enabled feature names
...
Result<void> action(const Parse &parse_args) {
    ...
    auto generate_build = [&](buildwriters::BaseWriter &writer) -> Result<void> {
        ...
        GeneratedVariables variables = writeVariables(config, writer, features, tc);
...
GeneratedVariables writeVariables(const catalyst::utils::yaml::Configuration &config,
    ...
    return {.cxxflags = std::move(cxxflags), .rpaths = std::move(rpath_flags)};
}
