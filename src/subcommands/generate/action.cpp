...
void writeVariables(const catalyst::utils::yaml::Configuration &config,
    ...
    auto proj_name_def = catalyst::toolchain::expand_template(
        tc.flags.define,
        {{"name", "CATALYST_PROJ_NAME"},
         {"value", "\\\"" + config.getString("manifest.name").value_or("name") + "\\\""}});
...
void writeRules(catalyst::generate::buildwriters::BaseWriter &writer, const catalyst::toolchain::ToolchainDef &tc) {
    ...
    void(writer.addRule("cxx_compile",
                        catalyst::toolchain::expand_template(tc.compiler.cxx.command,
                                                             {{"cxx", "$cxx"},
                                                              {"cxxflags", "$cxxflags"},
                                                              {"source", "$in"},
                                                              {"object", "$out"},
                        ...
                        "$out.d",
                        "gcc"));
    void(writer.addRule("cc_compile",
                        catalyst::toolchain::expand_template(tc.compiler.c.command,
                                                             {{"cc", "$cc"},
                                                              {"cflags", "$cflags"},
                                                              {"source", "$in"},
                                                              {"object", "$out"},
    ...
    void(writer.addRule(
        "static_link",
        catalyst::toolchain::expand_template(
            tc.archiver.command, {{"archiver", tc.archiver.executable}, {"objects", "$in"}, {"output", "$out"}}),
        "LINK $out"));
